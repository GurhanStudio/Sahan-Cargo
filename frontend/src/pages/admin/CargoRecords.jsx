    ...
    <div className="space-y-6">
      ...
      <div className="card">
        <form onSubmit={handleSearch} className="flex flex-wrap gap-3">
          <input className="input-field flex-1 min-w-[200px]" placeholder="Search by tracking #, sender, receiver..." value={search} onChange={e => setSearch(e.target.value)} />
          <select className="select-field w-auto" value={statusFilter} onChange={e => { setStatusFilter(e.target.value); setPage(1); }}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-800/50">
              {data.cargos.map(c => (
                <tr key={c.id} className="hover:bg-gray-800/30 transition-colors">
                  ...
                  <td className="table-cell">
                    <span className={`status-badge ${c.priority === 'HIGH_VALUE' ? 'bg-red-900/50 text-red-300' : 'bg-gray-700 text-gray-400'}`}>{c.priority}</span>
                  </td>
              ...
              {data.cargos.length === 0 && <tr><td colSpan="8" className="table-cell text-center text-gray-500 py-8">No cargo records found</td></tr>}
            </tbody>
          </table>
        </div>
        {data.totalPages > 1 && (
